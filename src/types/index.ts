...
export interface ModuleSymbol {
  ...
  jsdoc?: JSDocComment;           // 新增：结构化 JSDoc
...
// 类型定义（interface、type、enum、class、alias）
...
export interface GenericParam {
  ...
  default?: string;
  constraint?: string;
  variance?: 'in' | 'out' | 'inout';  // 变型: contravariant/covariant/bivariant
}
...
export interface MappedTypeInfo {
  ...
  constraint: string;
  valueType: string;
  asType?: string;  // as 关键字重映射: [K in T as NewKey]
  modifiers?: ('readonly' | 'optional' | 'nullable')[];
  readonly?: 'add' | 'remove';
  optional?: 'add' | 'remove';
}

// 模板字面量类型 `prefix-${T}-suffix`
...
export interface AdvancedTypeInfo {
  ...
  indexedAccessTypes?: string[];  // T[K]
  inferredTypes?: string[];       // infer R
}
