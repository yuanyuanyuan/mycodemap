...
import { ANALYZE_PARSE_OPTIONS } from '../../cli/commands/analyze-options.js';
import {
  PUBLIC_INTENTS,
  type AnalyzeArgs,
  type IntentType,
...
import { resolveDataPath, resolveOutputDir } from '../../cli/paths.js';
...
export async function executeAnalyzeTool(
  args: AnalyzeArgs
): Promise<ContractToolExecutionResult<CodemapOutput>> {
  ...
  try {
    ...
    if (args.structured) {
      ...
      if (structuredOutput.results) {
        structuredOutput.results = structuredOutput.results.map(entry => {
 ...
 */
export function formatAnalyzeHuman(output: CodemapOutput): string {
  const lines: string[] = [];
