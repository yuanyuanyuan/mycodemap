...
<required_reading>
@/data/codemap/.claude/get-shit-done/references/domain-probes.md
@/data/codemap/.claude/get-shit-done/references/gate-prompts.md
@/data/codemap/.claude/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
AGENT_SKILLS_ADVISOR=$(node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
- Read and execute @/data/codemap/.claude/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
```bash
TODO_MATCHES=$(node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="/data/codemap/.claude/get-shit-done/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
```bash
PROFILE_CONTENT=$(cat "/data/codemap/.claude/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @/data/codemap/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
**Full step-by-step instructions:** @/data/codemap/.claude/get-shit-done/workflows/discuss-phase-power.md
