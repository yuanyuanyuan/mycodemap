...
```bash
node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" audit-open 2>/dev/null
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**If doesn't exist:** Create from template at `/data/codemap/.claude/get-shit-done/templates/retrospective.md`.
...
**Commit:**
```bash
node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(node "/data/codemap/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
