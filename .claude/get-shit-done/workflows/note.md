...
- **Global scope**: `/data/codemap/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/data/codemap/.claude/notes/`)
...
2. Glob `/data/codemap/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/data/codemap/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/data/codemap/.claude/notes/` — works in any directory
