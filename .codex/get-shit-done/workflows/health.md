...
```bash
node "/data/codemap/.codex/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "/data/codemap/.codex/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
if [ -d "$TASKS_DIR" ]; then
  ...
  if [ "$STALE_COUNT" -gt 0 ]; then
    ...
    echo "   Run: rm -rf /data/codemap/.codex/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
