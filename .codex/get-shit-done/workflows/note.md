...
- **Global scope**: `/data/codemap/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/data/codemap/.codex/notes/`)
...
2. Glob `/data/codemap/.codex/notes/*.md` (if directory exists) — global notes
...
Global (/data/codemap/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `/data/codemap/.codex/notes/` — works in any directory
