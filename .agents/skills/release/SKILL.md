---
name: release
description: Codex runtime 的 `/release v{X.Y}` 薄适配器。它只负责读取 `docs/rules/release.md`、复用 refusal cases / 双确认门 / helper delegation，不重建 `scripts/release.sh` 或 `.github/workflows/publish.yml`。
user-invocable: true
---

# Release

> 单一权威：先读 `docs/rules/release.md`。本 skill 只是 **Codex runtime adapter**，不是第二套发布手册。

## 适用场景

- 用户显式输入 `/release v{X.Y}`
- 用户要求在 Codex runtime 下执行 release readiness 检查
- 用户要求核对 milestone ↔ npm version 绑定、确认门或失败处理

## 绝对边界

- 发布属于 **L3**；没有用户显式确认前，**不得自主发布**
- 没有用户显式确认前，**不得**执行 `npm version`、git commit、git tag、`git push` 或任何会触发 `.github/workflows/publish.yml` 的动作
- 本 skill 是 **thin adapter / thin orchestrator**：只验证、展示、确认、委托
...
如需在 release 之后做**只读**状态复核，提示用户使用：
...
- 本 skill 只负责提示，不自动运行 `publish-status`
- `publish-status` 是 follow-up observability，不是第二条 release 路径
- 如果无法提供精确 `tag + sha`，就不要猜“最新一条 publish run”
