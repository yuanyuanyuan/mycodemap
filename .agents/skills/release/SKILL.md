---
name: release
description: Codex runtime 的 `/release v{X.Y}` 薄适配器。包含发布编排、发布验证、失败排查三个独立执行面。只负责读取 `docs/rules/release.md`、复用 refusal cases / 双确认门 / helper delegation，不重建 `scripts/release.sh` 或 `.github/workflows/publish.yml`。
user-invocable: true
---
...
优先使用 GitHub API 或 `gh` CLI（如果可用）：

```bash
# 使用 gh CLI（推荐）
gh run list --repo {owner}/{repo} --workflow "Publish to NPM" -L 3
gh run list --repo {owner}/{repo} --workflow "CI Gateway" -L 3
